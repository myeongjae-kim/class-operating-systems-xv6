...
main(int argc, char *argv[])
{
   ...
   6:	cd 80                	int    $0x80
    exit();
   8:	e8 25 02 00 00       	call   232 <exit>
   d:	66 90                	xchg   %ax,%ax
   f:	90                   	nop
...
00000302 <thread_exit>:
SYSCALL(thread_exit)
 302:	b8 1c 00 00 00       	mov    $0x1c,%eax
 307:	cd 40                	int    $0x40
 309:	c3                   	ret    

0000030a <thread_join>:
SYSCALL(thread_join)
...
putc(int fd, char c)
{
 ...
 39a:	e8 b3 fe ff ff       	call   252 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 406:	e8 47 fe ff ff       	call   252 <write>
...
putc(int fd, char c)
{
 ...
 476:	e8 d7 fd ff ff       	call   252 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 493:	e8 ba fd ff ff       	call   252 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 5a0:	e8 ad fc ff ff       	call   252 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 6aa:	e8 0b fc ff ff       	call   2ba <sbrk>
  if(p == (char*)-1)
