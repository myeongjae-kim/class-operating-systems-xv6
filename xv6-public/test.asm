...
printint(int fd, int xx, int base, int sgn)
{
 ...
 369:	0f b6 92 5a 07 00 00 	movzbl 0x75a(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4ff:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 501:	b8 53 07 00 00       	mov    $0x753,%eax
...
free(void *ap)
{
 ...
 62f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 632:	a3 d0 09 00 00       	mov    %eax,0x9d0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 659:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 65c:	8b 1d d0 09 00 00    	mov    0x9d0,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 699:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 69b:	3b 15 d0 09 00 00    	cmp    0x9d0,%edx
...
morecore(uint nu)
{
 ...
 6cd:	e8 ee fe ff ff       	call   5c0 <free>
  return freep;
 6d2:	8b 15 d0 09 00 00    	mov    0x9d0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 70c:	eb e9                	jmp    6f7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 70e:	c7 05 d0 09 00 00 d4 	movl   $0x9d4,0x9d0
 715:	09 00 00 
    base.s.size = 0;
 718:	ba d4 09 00 00       	mov    $0x9d4,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 71d:	c7 05 d4 09 00 00 d4 	movl   $0x9d4,0x9d4
 724:	09 00 00 
    base.s.size = 0;
 727:	c7 05 d8 09 00 00 00 	movl   $0x0,0x9d8
 72e:	00 00 00 
