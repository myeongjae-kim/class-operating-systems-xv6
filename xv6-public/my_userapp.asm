...
int main(int argc, char *argv[])
{
   ...
   9:	c7 04 24 56 07 00 00 	movl   $0x756,(%esp)
  10:	e8 ed 02 00 00       	call   302 <my_syscall>
    printf(1, "Return value : 0x%x\n", ret_val);
  15:	c7 44 24 04 61 07 00 	movl   $0x761,0x4(%esp)
  1c:	00 
...
putc(int fd, char c)
{
 ...
 3ca:	e8 b3 fe ff ff       	call   282 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 436:	e8 47 fe ff ff       	call   282 <write>
...
putc(int fd, char c)
{
 ...
 4a6:	e8 d7 fd ff ff       	call   282 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 4c3:	e8 ba fd ff ff       	call   282 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 5d0:	e8 ad fc ff ff       	call   282 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 6da:	e8 0b fc ff ff       	call   2ea <sbrk>
  if(p == (char*)-1)
